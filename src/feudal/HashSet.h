...
public:
    ...
    HopscotchHashSet( size_type cap, HCF<T,H,C,F>& hcf )
...
public:
    ...
    void addDangerously( const_reference val )
    { size_t hash = mHCF.hash(val);
...
private:
    ...
    { HHS* pHi = new HHS(mInnerCapacity,mHCF);
      pHi->lock();
      ...
      { const_reference val = *itr;
        ...
        (((hash2^(hash2>>32))&mask)?pHi:&tmp)->insertV(val,hash2); }
