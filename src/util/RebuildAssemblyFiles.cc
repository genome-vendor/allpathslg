...
int main( int argc, char *argv[] )
{
  ...
  if ( ! IsRegularFile( IN_HEAD + ".assembly.fastg" ) ) {
	...
	fcontigs[i].AsEfasta( econtigs[i] );
      }
	...
	efasta ef;
	fcontigs_c[ifc].AsEfasta( ef );
	econtigs.push_back( ef );
	ForceAssertEq( fcontigs.back().Length1(), econtigs.back().Length1() );
	ForceAssertEq( fcontigs.back().MinLength(), econtigs.back().MinLength() );
	ForceAssertEq( fcontigs.back().MaxLength(), econtigs.back().MaxLength() );
      }
  ...
  for ( size_t i = 0; i < fcontigs.size(); i++ ){
    ...
    ForceAssertEq( fcontigs[i].MinLength(), econtigs[i].MinLength() );
    ForceAssertEq( fcontigs[i].MaxLength(), econtigs[i].MaxLength() );
  ...
  vec<efasta>   flattened_efasta(fcontigs.size());
  
  {
    ...
    for (size_t it = 0; it < fcontigs.size(); it++) {
      ...
      fcontigs[it].AsEfasta(flattened_efasta[it]);
