...
Assembly::Assembly( const String in_superb_file, const String in_contigs_file, const String in_scaff_graph_file ){
  ...
  if ( in_contigs_file.Contains(".efasta") ) {
    ...
    for ( size_t i = 0; i < fastgs_.size(); i++ )
      fastgs_[i].AsEfasta( efastas_[i] );
  }

  tigMap_.resize( efastas_.size() );
  for ( int tid = 0; tid < efastas_.isize(); tid++ )
...
Assembly::Assembly( const vec<superb>& scaffoldsIn, const vec<efasta>& efastasIn, const vec<String>* scaffMap, const vec<String>* tigMap, const digraphE<sepdev>* SGin ){
...
void Assembly::check_integrity() const{
  ...
  for ( size_t i = 0; i < efastas_.size( ); i++ ){    
    ...
    ForceAssertEq( fastgs_[i].MinLength(), efastas_[i].MinLength() );
    ForceAssertEq( fastgs_[i].MaxLength(), efastas_[i].MaxLength() );
    efasta fe;
    fastgs_[i].AsEfasta( fe );
    basevector b1, b2;
    fe.FlattenTo( b1 );
...
void Assembly::remove_small_scaffolds(const int min_scaffold_size) {
  ...
  for ( int si = 0; si < scaffolds_.isize(); si++ ){
    ...
    if ( scaffolds_.at(si).ReducedLength() < min_scaffold_size ){
      verts_to_remove.push_back(ri);
      scaffolds_.erase( scaffolds_.begin() + si );
      scaffMap_.erase( scaffMap_.begin() + si );
      SG_.DeleteEdgesAtVertex( si );
      si--;      
    }
  }
  SG_.RemoveEdgelessVertices( verts_to_remove );
  for ( int si = 0; si < scaffolds_.isize(); si++ )
    ForceAssertGe( scaffolds_.at(si).ReducedLength(), min_scaffold_size );
  ForceAssertEq( scaffolds_.isize(), SG_.N() );
...
void Assembly::dedup( const Bool Exact ) {
  ...
  vec<efasta> cefastas( efastas_.size() );
  vec<efasta> cefastas_rc( efastas_.size() );
