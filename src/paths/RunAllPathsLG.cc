...
int main( int argc, char *argv[] )
{
  ...
  CommandArgument_Int_OrDefault_Valid_Doc(ASSISTED_PATCHING, 0, "[0,2]",
    "use reference to guide contig patching prior. "
    "(0 - do not assist;  1 - assist by aligning contigs; 2 - assist by alignining unibases) ");
  ...
  CommandArgument_Bool_OrDefault(TAG_CIRCULAR_SCAFFODLS_CLIP, False);
...
// MakeDepend: dependency Jumpomatic
      make.AddRule( FilesIn( run_dir, current_unipaths + ".jumpomatic"
			       ...
			       + ToString(K), "jump_reads_filt.fastb" ),
...
// MakeDepend: dependency TagCircularScaffolds
  make.AddRule( FilesIn( sub_dir, current_scaffolds + ".tag.{contigs.fasta,contigs.vecfasta,contigs.fastb,assembly.fasta,superb,readlocs,rings}" ),
		...
		+ ARG( CLIP, TAG_CIRCULAR_SCAFFODLS_CLIP ),
