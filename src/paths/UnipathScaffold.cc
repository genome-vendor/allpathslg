...
template void digraphE<linklet>::Initialize(const vec<vec<int> >&, const vec<vec<int> >&, const vec<linklet>&, const vec<vec<int> >&, const vec<vec<int> >&);
...
template digraphE<linklet>::edge_label_info::edge_label_info(digraphE<linklet>::edge_label_info::ConstructorBehavior, unsigned char, unsigned char, vec<FeudalString<char, std::char_traits<char> > > const*);
