...
template void digraphE<ulink>::AddEdge(const int, const int, const ulink&);
...
template void digraphE<ulink>::Initialize( const vec<vec<int> >&, const vec<vec<int> >&, const vec<ulink>&, const vec<vec<int> >&, const vec<vec<int> >&);
template int digraphE<ulink>::InputFromOutputTo(int, int) const;
template void digraphE<ulink>::RemoveDeadEdgeObjects();
