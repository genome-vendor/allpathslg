...
class efasta : public String {
     ...
     Bool ExpandTo( vec< vec<basevector> >& G, vec< vec<int> >& paths, const int max_count = -1 ) const;
     ...
     // arase the segment [start,stop) where start and stop are determined by the first choice in brackets
     void Erase1( const int start, const int stop );

      // extract the segment [start,stop) where start and stop are determined by the first choice in brackets
     ...
     void GetKmers( const int K, vecbasevector& kmers, 
      ...
      // Prints efasta ambiguites in fasta format: ">evant_number-alternative-number\n" 
...
void LoadEfastaIntoStrings( const String& fn, vec<efasta>& x, vec<String>& headers );
