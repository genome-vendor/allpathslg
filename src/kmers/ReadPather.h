...
public:
    ...
    { Assert(isNull()); ForceAssertLt(edgeOffset,1u<<24);
    ...
    { Assert(isNull()); if ( mEdgeOffset < (1u<<24)-1u ) ++mEdgeOffset;
      return mEdgeOffset; }
...
public:
    ...
    /// Copy entries with counts in the given range into another dictionary.
    void extractByKmerCount( KmerDict* pDict, size_t minCount,
                                                size_t maxCount = ~0ul )
    { for ( OCItr oItr(cbegin()), oEnd(cend()); oItr != oEnd; ++oItr )
...
private:
    ...
    void copyEntry( Entry const& entry )
    { mKSet.addDangerously(entry); }
...
{
public:
    EdgeDesc( EdgeID const& edgeID, CanonicalForm::Value status )
    : mEdgeID(edgeID), mStatus(status) {}

    // compiler-supplied copying and destructor are OK

    EdgeID const& getEdgeID() const { return mEdgeID; }
    CanonicalForm::Value getStatus() const { return mStatus; }

    EdgeDesc operator~() const
    { return EdgeDesc(mEdgeID,CanonicalForm::complement(mStatus)); }
...
public:
    ...
    { UnipathEdge const& edge = mGraph.getEdge(ed.getEdgeID());
      EdgeID id;
      CanonicalForm::Value status = CanonicalForm::FWD;
...
EdgeList PathCollection<K>::getEdgeList( size_t readId ) const
{
    ...
    if ( nSegs )
    {
        ...
        CanonicalForm::Value status = isPalindrome ?
