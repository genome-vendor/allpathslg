...
{    if ( from_.size( ) != to_.size( ) )
          DIGRAPH_INVALID( "sizes of from_ and to_ are different" );
     for ( int v = 0; v < N( ); v++ )
     {    if ( !from_[v].Ordered( ) )
          {    DIGRAPH_INVALID( "from_[" << v << "] is not ordered" );    }
          if ( !to_[v].Ordered( ) )
          {    DIGRAPH_INVALID( "to_[" << v << "] is not ordered" );    }    }
     for ( int v = 0; v < N( ); v++ )
     {    if ( !to_[v].Ordered( ) )
               DIGRAPH_INVALID( "to_[" << v << "] is not sorted" );    }
     for ( int v = 0; v < N( ); v++ )
     {    for ( int j = 0; j < from_[v].isize( ); j++ )
          {    int w = from_[v][j];
               if ( w < 0 )
...
{    path.clear( );
     ...
     {    dist[start] = 0;
          ...
          {    pair<int,int> p = *X.begin( );
               int min_dist = p.first, u = p.second;
               X.erase( X.begin( ) );
               Q[u] = False;
...
template digraphE<int>::digraphE(const vec<vec<int> >&, const vec<vec<int> >&, const vec<int>&, const vec<vec<int> >&, const vec<vec<int> >&);
template void digraphE<int>::AddEdge(const int, const int, const int&);
...
template Bool digraphE<int>::EdgePaths(const int, const int, vec<vec<int> >&, const int, const int, const int);
...
template void digraphE<int>::Initialize(const vec<vec<int> >&, const vec<vec<int> >&, const vec<int>&, const vec<vec<int> >&, const vec<vec<int> >&);
...
template void digraphE<int>::PrettyDOT( ostream& out, 
     ...
     const vec<Bool>* dashed ) const;

template void digraphE<int>::RemoveDeadEdgeObjects();
...
template digraphE<IntPair>::digraphE(const vec<vec<int> >&, const vec<vec<int> >&, const vec<IntPair>&, const vec<vec<int> >&, const vec<vec<int> >&);
...
template void digraphE<IntPair>::Initialize(vec<vec<int> > const&, vec<vec<int> > const&, vec<IntPair> const&, vec<vec<int> > const&, vec<vec<int> > const&);
