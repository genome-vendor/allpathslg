...
template<class E> class digraphE : public digraph {
     ...
     // Constructor 5 was frought with peril, and has been deleted.
...
template <> void digraphE<int>::DOT_vel( ostream& out, const vec<String> & vertex_labels );
template <> void digraphE<int>::ShortestPath( const int start, const int stop, vec<int>& path ) const;
extern template class digraphE<int>;
extern template class digraphE<VertexPair>;

template <class E> struct Serializability< digraphE<E> >
{ typedef SelfSerializable type; };
