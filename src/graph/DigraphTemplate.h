...
template<class E> int digraphE<E>::UsedCount( ) const
{    vec<Bool> used;
     Used(used);
     return Sum(used);    }
...
{    digraph(*this).TestValid( );
     if ( from_edge_obj_.size( ) != to_edge_obj_.size( ) )
         DIGRAPH_INVALID( "sizes of from_edge_obj_ and to_edge_obj_ are different" );
     if ( from_.size( ) != from_edge_obj_.size( ) )
         DIGRAPH_INVALID( "sizes of from_ and from_edge_obj_ are different" );
     for ( int v = 0; v < N( ); v++ )
     ...
     {    for ( int j = 0; j < from_[v].isize( ); j++ )
          {    int w = from_[v][j];
               ...
               if ( ei < 0 || ei >= EdgeObjectCount( ) )
                    DIGRAPH_INVALID( "Illegal from_edge_obj value." );
               Bool found = False;
     ...
     {    for ( int j = 0; j < to_[v].isize( ); j++ )
          {    int w = to_[v][j];
               ...
               if ( ei < 0 || ei >= EdgeObjectCount( ) )
                    DIGRAPH_INVALID( "Illegal to_edge_obj value." );
               Bool found = False;
...
digraphE<E>::PrettyDOT( ostream& out, const vec<double>& lengths,
     ...
     const vec<int> *verticesToPrint, const vec<Bool>* dashed ) const
{
  // Set up output.
  out << "digraph G {\n\n";
...
template<class E> bool EqualExceptEdgeObjectOrder( 
     const digraphE<E>& g1, const digraphE<E>& g2 )
{
